...
object Parser {
  ...
  val keyValuePair: P[Field] = P(identifier ~ sp ~ ":" ~ sp ~ expr).map { case (key, value) => Field(key.id, value) }
  val record: P[Record] = P("{" ~/ sp ~ keyValuePair.rep(0, "," ~ sp) ~ sp ~ "}").map(kvp => Record(kvp.toList))

  lazy val parens: P[Expr] = P("(" ~/ expr ~/ ")")
  lazy val simple_exp: P[Expr] = P(number | bool | identifier | record | parens)
